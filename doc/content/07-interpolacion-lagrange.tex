...
Según \cite{Suli2003}, la interpolación polinómica implica encontrar un polinomio que coincida exactamente con cierta información que tenemos sobre una función de valor real $f$ de una única variable real $x$. Esta información, continúa el autor, puede estar en forma de valores $f(x_0), \dots, f(x_n)$ de la función $f$ en un conjunto finito de puntos $\{x_0, \dots, x_n\}$ en la recta real, y el polinomio correspondiente se llama entonces el \textbf{polinomio de interpolación de Lagrange}\textsuperscript{1} o, siempre que $f$ sea diferenciable, puede incluir valores de la derivada de $f$ en estos puntos, en cuyo caso el polinomio asociado se denomina \textbf{polinomio de interpolación de Hermite}.


%Según \cite{} la fórmula de la interpolación de Lagrange es como sigue: 
...
% Interpolacion de Lagrange
    ...
    function lagrangeInterpolation(filename, col_x, col_y, num_points)
        ...
        fplot(lagrangePoly, [min(x_data) max(x_data)], 'r-', 'LineWidth', 2);
...
Con base al dataset mostrado en la figura \ref{fig:dataset}, podemos hacer uso del algoritmo de interpolación de Lagrange para obtener un polinomio que pase exactamente por ciertos puntos seleccionados.

Para el presente ejemplo, seleccionaremos 5 puntos distribuidos equitativamente del dataset en las columnas 6 y 7 (Nota Algoritmos VS Horas de estudio). El resultado del polinomio computado es, como se muestra en la figura \ref{fig:exec:lagrange}, como sigue.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.38]{resources/interpolacion/exec-lagrange.png}
	\caption{Ejecución del algoritmo de interpolación de Lagrange con 5 puntos seleccionados del dataset de prueba}
	\label{fig:exec:lagrange}
\end{figure}

La gráfica del polinomio interpolador se puede observar en la figura \ref{fig:plot:lagrange}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{resources/interpolacion/plot-lagrange.png}
	\caption{Gráfica del polinomio interpolador de Lagrange con 5 puntos seleccionados del dataset de prueba}
	\label{fig:plot:lagrange}
\end{figure}
