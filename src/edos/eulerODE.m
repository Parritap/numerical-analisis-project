...
function eulerODE_method()
    try
        ...
        if ~isscalar(y0) || ~isscalar(t0) || ~isscalar(tf) || ~isscalar(h)
            error('Las condiciones iniciales, tiempo inicial, tiempo final y tamano del paso deben ser escalares.');
        end

        if tf <= t0 || h <= 0
            error('El tiempo final debe ser mayor que el tiempo inicial y el tamano del paso debe ser positivo.');
        end
        ...
        error('Error en la entrada de datos. Asegurese de ingresar la ecuacion diferencial, condiciones iniciales, tiempo inicial, tiempo final y tamano del paso correctamente.');
    end
end
